...
% Figure 6 - aftereffect for each experiment
...
ckmeanplotcompact(BiasVAE(:,CondOrder),2,1,[],Colorvector);
ylabel('slope');
...
% add Bayesfactors
BF = round(Stat_each_VAE(CondOrder));
for cond=1:4
  usecond = CondOrder(cond);
...
% add Bayesfactors
BF = round(Stat_each_VAE);

ax = subplot(2,3,6);
ckmeanplotcompact(BiasVAE(:,CondOrder),2,1,[],Colorvector(CondOrder,:));
ylabel('slope');
